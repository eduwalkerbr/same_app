...
<div class="container" style="margin-top: 100px;">
  <div class="card text-center">
    <div class="card-body" style="background-color: #0046AD; color: white;box-shadow: 5px 5px 5px rgba(0,0, 139);">
      ...
      <p class="card-text">Segue abaixo a lista de Municípios para Visualização e ou Edição.</p>
    </div>
  </div>
  @csrf
  <br>
    ...
    <div class="accordion-item">
      ...
      <div id="panelsStayOpen-collapseOne" class="accordion-collapse collapse" aria-labelledby="panelsStayOpen-headingOne">
        <div class="accordion-body">
          <form id="form_filter" name="form_filter" action="{{ route('municipio.filtrar') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row justify-content-center" style="color:black;font-size:15px;">
              <div class=" col-md-8">
                <div class="form-group">
                  <label for="nome">Nome</label>
                  <input type="text" class="form-control" id="nome" name="nome" placeholder="Nome do Município" value="">
                </div>
              </div>
              <div class=" col-md-4">
                <div class="form-group">
  ...
  <table class="table responsive-md">
    ...
    <tbody>
      ...
      <tr>
        <th style="font-weight: normal;font-size:14px;" scope="row">{{$municipio->id}}</th>
        <td style="font-weight: normal;font-size:14px;">{{$municipio->nome}}</td>
        <td style="font-weight: normal;font-size:14px;">{{$municipio->uf}}</td>
        <td style="font-weight: normal;font-size:14px;text-align:center;">{{$municipio->SAME}}</td>
        <td style="font-weight: normal; text-align:center;font-size:14px;">{{$municipio->updated_at->format('d/m/Y H:i:s')}}</td>
        <td style="text-align:center;">
          <a href="{{ route('cadastro_municipio') }}">
            <button style="font-weight: normal;background-color:#f9821E;border-color:#f9821E;" class="btn-primary">Novo</button>
          </a>
          <a href="{{ route('municipio.edit', ['id' => $municipio->id, 'anosame' => $municipio->SAME]) }}">
            <button style="font-weight: normal;background-color:black;border-color:black;" class="btn-primary">Editar</button>
          </a>
          @if(isset($municipio) && $municipio->status == 'Ativo')
          <a href="{{ route('municipio.inativar',['id' => $municipio->id, 'anosame' => $municipio->SAME]) }}">
            <button style="font-weight: normal;" class="btn-danger">Inativar</button>
          </a>
          @else
          <a href="{{ route('municipio.ativar',['id' => $municipio->id, 'anosame' => $municipio->SAME]) }}">
  ...
  <div class="row justify-content-center">
    {{ $municipios->appends(Request::except('page'))->links() }}
  </div>
