...
<div class="container" style="margin-top: 100px;">
  <div class="card text-center">
    <div class="card-body" style="background-color: #0046AD; color: white;box-shadow: 5px 5px 5px rgba(0,0, 139);">
      <h5 class="card-title">Lista de Habilidades</h5>
      <p class="card-text">Segue abaixo a lista de Habilidades para Visualização e ou Edição.</p>
    </div>
  </div>
  @csrf
  <br>
    ...
    <div class="accordion-item">
      ...
      <div id="panelsStayOpen-collapseOne" class="accordion-collapse collapse" aria-labelledby="panelsStayOpen-headingOne">
        <div class="accordion-body">
          <form id="form_filter" name="form_filter" action="{{ route('habilidade.filtrar') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row justify-content-center" style="color:black;font-size:15px;">
              <div class=" col-md-8">
                <div class="form-group">
                  <label for="desc">Descrição</label>
                  <input type="text" class="form-control" id="desc" name="desc" placeholder="Descrição da Habilidade" value="">
                </div>
              </div>
              <div class=" col-md-4">
                <div class="form-group">
  ...
  <table class="table responsive-md">
    ...
    <tbody>
      ...
      @php
      $disciplina = $habilidade->find($habilidade->id)->relDisciplinas;
      @endphp
      <tr>
        <th style="font-weight: normal;font-size:14px;" scope="row">{{$habilidade->id}}</th>
        <td style="font-weight: normal;font-size:14px;width:550px;">{{$habilidade->desc}}</td>
        <td style="font-weight: normal;font-size:14px;">{{$disciplina->desc}}</td>
        <td style="font-weight: normal; text-align:center;font-size:14px;">{{$habilidade->updated_at->format('d/m/Y H:i:s')}}</td>
        <td style="text-align:center;">
          <a href="{{ route('cadastro_habilidade') }}">
            <button style="font-weight: normal;background-color:#f9821E;border-color:#f9821E;" class="btn-primary">Novo</button>
          </a>
          <a href="{{ route('habilidade.edit', $habilidade->id) }}">
  ...
  <div class="row justify-content-center">
    {{ $habilidades->appends(Request::except('page'))->links() }}
  </div>
