<div class="card text-center" style="box-shadow: 5px 5px 5px rgba(156,163,175);">
    ...
    <div class="card-header" style="text-align: justify;background-color: white; border-bottom:none;font-size:15px;color:black;font-weight:bold;">
        <i class="fa-solid fa-chalkboard-user"></i> &emsp; {{$turma_selecionada[0]->DESCR_TURMA}}
    </div>
    <!------------------------------------ Título Card ------------------->
    @php
    $ano = substr($turma_selecionada[0]->DESCR_TURMA, 1, 2);
    @endphp
    <div class="card-body" style="padding-top:0.5rem;padding-bottom:0.5rem;">
        <p class="card-text" style="font-size:13px;color:black;border-bottom:none;text-align:justify;font-weight:bold;">Seguem abaixo alguns dados relacionados a Turma.</p>
        <!------------------------------------ Cards ------------------->
        @foreach ($dados_base_turma as $group)
        ...
        <p style="color:rgba(107,114,128);font-size: 13px;text-align:justify;margin-top:30px;">
            * Na coluna da direita, temos a proficiência Média das turmas do {{$ano[0]}}º Ano, comparada a turma principal.
        </p>
        <!------------------------------------ Legenda ------------------->
