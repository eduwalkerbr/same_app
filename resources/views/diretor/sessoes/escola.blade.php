<div class="card text-center" style="box-shadow: 5px 5px 5px rgba(156,163,175);">
    ...
    <div class="card-header" style="text-align: justify;background-color: white; border-bottom:none;font-size:15px;color:black;font-weight:bold;">
        <i class="fa-solid fa-chalkboard-user"></i> &emsp; {{$escola_selecionada[0]->nome}}
    </div>
    <!------------------------------------ Título Card ------------------->
    <div class="card-body" style="padding-top:0.5rem;padding-bottom:0.5rem;">
        <p class="card-text" style="font-size:13px;color:black;border-bottom:none;text-align:justify;font-weight:bold;">Seguem abaixo alguns dados relacionado a Escola.</p>
        <!------------------------------------ Cards ------------------->
        @foreach ($dados_base_escola as $group)
        <div class="row justify-content-center">

            @foreach ($group as $dados_base)
            @foreach($legendas as $legenda)
            @php
            if(number_format($dados_base->percentual_escola, 0, '.', '') >= $legenda->valor_inicial && number_format($dados_base->percentual_escola, 0, '.', '') <= $legenda->valor_final){
                $corFundo = $legenda->cor_fundo;
                $corLetra = $legenda->cor_letra;
                }
                @endphp
                @endforeach
                <div class="col-md-6" style="margin-top:20px;border: 1px solid white;background-color:white;padding-left:0px;padding-right:0px;">
                    <div class="card text-center" style="background-color:{{$corFundo}};border: 1px solid #f0f8ff;box-shadow: 5px 5px 5px rgba(156,163,175);">
                        ...
                        <div class="card-body" style="padding-bottom:0.5em;padding-top:0.5em;">
                            <h5 class="card-title" style="text-align: center;background-color:{{$corFundo}}; border-radius:50%;border-bottom:none;font-size:25px;color:{{$corLetra}};font-weight:bold;"><?php echo number_format($dados_base->percentual_escola, 0, '.', '') ?>%</h5>
                        </div>
                    </div>
                </div>
                @endforeach
        ...
        @endforeach
        <!------------------------------------ Cards ------------------->
        <p style="color:rgba(107,114,128);font-size: 13px;text-align:justify;margin-top:30px;" id="disciplina_detalhado">
            * Os dados correspondem a uma comparação entra a proficiência Média das Escolas do Município, em relação a escola selecionada.
        </p>
        <!------------------------------------ Legenda ------------------->
